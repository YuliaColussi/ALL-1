...
var anime = (function() {
  var x = {
        ...
        b = {
          ...
          Elastic: function(a, b) {
            ...
            return -(
              Math.pow(2, 10 * h) *
              Math.sin(
                (2 * (h - (f / (2 * Math.PI)) * Math.asin(1)) * Math.PI) / f
              )
            );
          },
          Back: function(a) {
    ...
    E = function(a, b) {
      if ((e.node(a) || e.svg(a)) && B(M, b)) return "transform";
      if ((e.node(a) || e.svg(a)) && "transform" !== b && v(a, b)) return "css";
      if ((e.node(a) || e.svg(a)) && (a.getAttribute(b) || a[b]))
        return "attribute";
    ...
    Q = function(a, b) {
      var c = -1 < b.indexOf("scale") ? 1 : 0,
        d = a.style.transform;
      if (!d) return c;
      for (var f = /(\w+)\((.+?)\)/g, h = [], e = [], p = []; (h = f.exec(d)); )
        e.push(h[1]), p.push(h[2]);
    ...
    T = function(a, b) {
      ...
      for (d in a)
        if (!x.hasOwnProperty(d) && "targets" !== d) {
          var f = e.object(a[d]) ? u(a[d]) : { value: a[d] };
          f.name = d;
          c.push(q(f, b));
        }
      return c;
      ...
      a.forEach(function(d, f) {
        ...
        return b.forEach(function(b) {
          ...
          if (p) {
            ...
            g.delay =
              (e.func(g.delay) ? g.delay(h, f, a.length) : g.delay) / l.speed;
            g.duration =
              (e.func(g.duration) ? g.duration(h, f, a.length) : g.duration) /
              l.speed;
            c.push(g);
          }
        });
    ...
    J = function(a) {
      ...
      a.tweens.forEach(function(a) {
        if ("css" === a.type || "transform" === a.type)
          b.push("css" === a.type ? z(a.name) : "transform"),
            a.animatables.forEach(function(a) {
              c.push(a.target);
            });
      });
      return { properties: C(b).join(", "), elements: C(c) };
    },
    W = function(a) {
      var b = J(a);
    ...
    K = function(a, b) {
      ...
      if (c)
        for (var d in c)
          a.animatables[d].target.style.transform = c[d].join(" ");
    ...
    L = function(a) {
      ...
      b.duration = b.tweens.length
        ? Math.max.apply(
            Math,
            b.tweens.map(function(a) {
        ...
        c = {
          tick: function() {
            if (b.running) {
              ...
              a.begin &&
                c.current >= a.delay &&
                (a.begin(b), (a.begin = void 0));
              c.current >= b.duration
                ? (a.loop
                    ? ((c.start = +new Date()),
                      "alternate" === a.direction && w(b, !0),
                      e.number(a.loop) && a.loop--,
                      (c.raf = requestAnimationFrame(c.tick)))
                    : ((b.ended = !0), a.complete && a.complete(b), b.pause()),
                  (c.last = 0))
                : (c.raf = requestAnimationFrame(c.tick));
            }
          }
  ...
  l.remove = function(a) {
    ...
    for (var b = m.length - 1; 0 <= b; b--)
      for (var c = m[b], d = c.tweens.length - 1; 0 <= d; d--)
        for (var f = c.tweens[d], h = f.animatables.length - 1; 0 <= h; h--)
          B(a, f.animatables[h].target) &&
            (f.animatables.splice(h, 1),
            f.animatables.length || c.tweens.splice(d, 1),
            c.tweens.length || c.pause());
  };
  l.easings = y;
  l.getValue = F;
  l.path = function(a) {
